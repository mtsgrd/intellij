...
public interface BlazeCompilerFlagsProcessor {
  ...
  ExtensionPointName<BlazeCompilerFlagsProcessor> EP_NAME =
      ExtensionPointName.create("com.google.idea.blaze.cpp.BlazeCompilerFlagsProcessor");

  /** True if this processor applies to the given project. */
  ...
  static List<String> process(Project project, List<String> compilerFlags) {
    List<BlazeCompilerFlagsProcessor> processors =
        Arrays.stream(EP_NAME.getExtensions())
            .filter(processor -> processor.isActive(project))
